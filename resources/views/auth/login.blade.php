...
<head>
    ...
    <title>{{ __('Sign In') }} - Lavalite ERP</title>
        ...
        <div class="w-full max-w-md">
            ...
            <div class="bg-white/5 backdrop-blur-lg border border-white/10 rounded-2xl p-8 shadow-2xl">
                <form method="POST" action="{{ route('login.submit') }}" class="space-y-6">
                    ...
                    <!-- Social Login -->
                    @php
                        $socialProviders = array_filter(config('social.providers', []), fn($p) => $p['enabled'] ?? false);
                    @endphp
                    
                    @if(count($socialProviders) > 0)
                    <div class="grid grid-cols-{{ min(count($socialProviders), 2) }} gap-3">
                        @foreach($socialProviders as $provider => $config)
                            <a 
                                href="{{ route('social.redirect', $provider) }}" 
                                class="flex items-center justify-center px-4 py-3 bg-white/5 hover:bg-white/10 border border-white/10 rounded-lg transition-all group"
                            >
                                @if($provider === 'google')
